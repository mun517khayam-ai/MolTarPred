#!/usr/bin/env python3
"""
V2 (fingerprints / STRICT database) run on the shared master query set.

Identical logic to V1's master_run script, but the reference library is V2's strict
grouped database (relation '=', binding assays, >0 & <=10000 nM). Same master query
molecules and same canonical ground truth, so results are directly comparable to V1.
...
DB_CSV   = f"{CODE}/v2/v2_chembl_36_database.csv"    # STRICT grouped DB
QUERY    = f"{CODE}/query_master_FDA_strictGT.csv"            # shared master queries
OUTDIR   = Path(__file__).parent
METHOD   = "V2_Morgan_Tanimoto_strictGT"
TOPK     = 10
# -----------------------------------------------------------------------------
